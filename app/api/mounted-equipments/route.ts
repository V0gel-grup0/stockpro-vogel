...
export async function GET() {
  try { return NextResponse.json({ sucesso: true, items: toJsonSafe(await prisma.mounted_equipments.findMany({ orderBy: { equipment_name: "asc" } })) }); }
...
export async function DELETE(request: Request) {
  try { const id = new URL(request.url).searchParams.get("id"); if (!id) return NextResponse.json({ sucesso: false, erro: "ID é obrigatório." }, { status: 400 }); await prisma.mounted_equipments.delete({ where: { id } }); return NextResponse.json({ sucesso: true }); }
